...
public class PowertrainManager
{
    ...
    public float[] ProcessTorqueRouting(WheelAssembly[] corners, float activeThrottle, float activeBrake, bool isAutomatic, float dt)
    {
        ...
        float downstreamInertiaShare = 0f;
        if (powerTrain.clutch.isLocked)
        {
            float ratio = powerTrain.transmission.GetTotalRatio();
            float totalDownstreamInertia = (powerTrain.engine._engineData.engineInertia * ratio * ratio) + powerTrain.transmission.transmissionData.transmissionInertia;

            int drivenWheels = drivetrain.drivetrainData.driveType == DriveType.AWD ? 4 : 2;
            downstreamInertiaShare = totalDownstreamInertia / drivenWheels;
        }
